...
User.forEach(el => {
  userAttributes.push({
    ...
    alamat: el.alamat,
    action: <div className="d-flex flex-wrap"><ModalEditUser classButtonModal="btn btn-success btn-sm fa fa-edit mx-1" id={el.id} modalName="Edit User" nama={el.nama} kelamin={el.kelamin} tempat={el.tempat} tanggal_lahir={el.tanggal_lahir} alamat={el.alamat}></ModalEditUser> <Button handleClick={() => handleDelete(el.id - 1)} classButton="btn btn-danger btn-sm fa fa-trash mx-1"></Button> <ModalTopupUser classButtonModal="btn btn-info btn-sm fa fa-money-bill-wave mx-1" modalName="Topup User" namatopup={el.nama}></ModalTopupUser> <Button handleClick={() => handleReset(el.id - 1)} classButton="btn btn-warning btn-sm fa fa-key mx-1"></Button></div>,
