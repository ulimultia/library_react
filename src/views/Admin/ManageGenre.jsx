    ...
    submitNow = (e) => {
        ...
        if(this.state.btn === "Tambah"){
            ...
            if(isValid === true){
                ...
                axios.post("http://localhost:8080/api/v1/genre/add", genreDto)
                .then((response) => {
                    this.setState({
                        modal: false,
                        namaGenre: "",
    ...
    handleDelete = (id) => {
        ...
        .then((willDelete) => {
            if (willDelete.isConfirmed) {
                axios.delete('http://localhost:8080/api/v1/genre/delete/' + id)
                .then((response) => {
                    this.setState({
                        genres: [],
                        newGenres: [],
    ...
    render() {
        return (
                            ...
                            <Row>
                                <Col xs="12" sm="12" className = "text-right">
                                    ...
                                    <Modal isOpen={this.state.modal} toggle={this.toggle} className="modal-sm" id="modalTambah">
                                        <ModalHeader toggle={this.toggle} style={{backgroundImage: "linear-gradient(to left, #44a08d, #093637)",color: "#ffffff"}}>Tambah Genre Buku</ModalHeader>
                                        <form id="form">
                                        <ModalBody className="mx-4">
                                            <FormGroup>
                                                <Input type="number" name="idGenre" id="idGenre" value={this.state.idGenre} hidden={true}/>
                                            </FormGroup>
                                            <FormGroup>
                                                <Label for="namaKategori">Genre</Label>
                                                <Input type="text" name="namaGenre" id="namaGenre" placeholder="Contoh: Horor"
            ...
            <Modal isOpen={this.state.modalEdit} toggle={this.toggleEdit} className="modal-sm">
                <ModalHeader toggle={this.toggleEdit} style={{backgroundImage: "linear-gradient(to left, #44a08d, #093637)",color: "#ffffff"}}>Edit Genre Buku</ModalHeader>
                <form id="form">
                <ModalBody className="mx-4">
                    <FormGroup>
                        <Input type="number" name="idGenre" id="idGenre" value={this.state.idGenre} hidden={true}/>
                    ...
                    <FormGroup>
                        <Label for="namaKategori">Genre</Label>
                        <Input type="text" name="editNamaGenre" id="editNamaGenre" placeholder="Contoh: Horor"
